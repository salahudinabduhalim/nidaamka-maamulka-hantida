...
document.addEventListener('DOMContentLoaded', () => {
    ...
    function renderData() {
        ...
        Object.values(inventory).forEach(q => {
            totalItems += q;
            if (q <= 10) lowStockCount++;
        });

        // Dashboard only counts low stock if there are actually items in the database
        // If masterItems is empty, everything should be 0.
        if (masterItems.length === 0) lowStockCount = 0;
    ...
    document.getElementById('in-existing-category').addEventListener('change', (e) => {
        ...
        const isBooks = ['books', 'book', 'buug', 'buugaag'].some(term => lowerCat.includes(term));
        ...
        if (catValue === 'NEW_CATEGORY') {
            ...
            // Listen for input on new category to detect "Books"
            newCatInput.addEventListener('input', (e) => {
                const newLowerCat = e.target.value.trim().toLowerCase();
                const newIsBooks = ['books', 'book', 'buug', 'buugaag'].some(term => newLowerCat.includes(term));
                if (newIsBooks) {
            ...
            const filteredItems = masterItems.filter(m => {
                const mCat = (m.category || '').trim().toLowerCase();
                return mCat === lowerCat;
            });
