...
def on_startup():
    ...
    with Session(engine) as session:
        # Force Reset: If old items exist, clear everything to start fresh with the 37 items
        # We detect old data by checking for items that aren't in our new list or a known old item
        old_data_check = session.exec(select(Activity).where(Activity.action.contains("office chair"))).first()
        if old_data_check:
            print("Old data detected. Wiping database for bulk reset...")
