...
def on_startup():
    ...
    with Session(engine) as session:
        # Force Reset: If old data OR duplicates exist, wipe and re-seed exactly 37 items.
        # Check for 'office chair' (old) OR if count is not 37 (duplicate/incomplete)
        item_count = len(session.exec(select(Item)).all())
        old_data_check = session.exec(select(Activity).where(Activity.action.contains("office chair"))).first()
        
        if old_data_check or (item_count > 0 and item_count != 37):
            print(f"Cleanup needed (Count: {item_count}). Wiping database...")
        ...
        # 2. Seed Bulk Items (if none exist)
        if not session.exec(select(Item)).first():
            ...
            for item_data in items_to_add:
                ...
                activity = Activity(
                    ...
                    user="System Admin",
                    comment="Automatic Seeding (Special Needs & Gender)",
                    status="Approved"
                )
