...
def on_startup():
    ...
    with Session(engine) as session:
        ...
        # 2. FINAL WIPE (Reset to 0)
        # We use raw SQL to ensure the tables are completely empty and stay empty.
        print("FORCED RESET: Wiping all items and activities to 0...")
        session.execute(text("DELETE FROM activity"))
        session.execute(text("DELETE FROM item"))
        session.commit()
        print("SYSTEM RESET TO ZERO.")
